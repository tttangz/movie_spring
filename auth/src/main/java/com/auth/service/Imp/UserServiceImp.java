package com.auth.service.Imp;


import cn.hutool.http.HttpUtil;
import cn.hutool.json.JSONObject;
import cn.hutool.json.JSONUtil;
import com.auth.db.dao.TbUserDao;
...
//        }
        ...
        public int registerUser(String registerCode, String code, String nickname, String photo) {
            if (registerCode.equals("000000")) {
                Integer num = userDao.haveRootUser();
                if (num==0) {
                    ...
                    messageClient.sendRegistMessage(id.toString());
                    return id;
                } else {
                    throw new GlobalException("无法绑定超级管理员账号");
                }
            } else if (!redisTemplate.hasKey(registerCode)) {
                //判断邀请码是否有效
                throw new GlobalException("不存在这个激活码");
            } else {
                ...
                if (row != 1) {
                    throw new GlobalException("账号激活失败");
                }
        ...
        public Integer login(String code) {
            ...
            Integer id = userDao.searchIdByOpenId(openId);
            if (id == null) {
                throw new GlobalException("帐户不存在");
            }
            messageClient.recvRegistMessage(id.toString());
            return id;
        }
...
//            } else {
//                throw new GlobalException("员工数据添加失败");
//            }
//        }
...
//            if (row != 1) {
//                throw new GlobalException("删除员工失败");
//            }
