...
public class MessageDao {
    ...
    public List<HashMap> searchMessageByPage(int userId, long start, int length) {
        ...
        Aggregation aggregation = Aggregation.newAggregation(
                //把json里通过tostring转换好的_id赋给id（临时变量）
                ...
                Aggregation.project("id", "msg", "sendTime","senderId","senderName","senderPhoto","uuid")
                        .and(item-> Document.parse("{$com.common.filter:{input:\"$ref\",as:\"item\",cond:{$eq:[\"$$item.receiverId\","+userId+"]}}}")).as("ref"),
        ...
        //表连接关系决定是一（message）对多（ref）
        list.forEach(one -> {
            //one代表message
            //refList代表refmessage  实现一对多
            List<MessageRefEntity> refList = (List<MessageRefEntity>) one.get("ref");
            //get0是因为限定了当前user，所以查出来的refList实际上只有一条数据
